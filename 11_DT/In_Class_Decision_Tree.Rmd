...
xx$workclass <- fct_collapse(xx$workclass, gov = c("Federal-gov", "Local-gov", "State-gov"), private = "Private", self_employed = c("Self-emp-inc", "Self-emp-not-inc"), Unpaid="Without-pay")
xx$education <- fct_collapse(xx$education, below_HS = c("1st-4th", "5th-6th", "7th-8th", "Preschool"), HS = c("9th", "10th", "11th", "12th", "HS-grad"), Associate = c("Assoc-acdm", "Assoc-voc"), Bachelor = c("Bachelors", "Some-college", "Prof-school"), Graduate = c("Masters", "Doctorate", "Prof-school"))
xx$native.country<- fct_collapse(xx$native.country, NorthA = c("United-States", "Canada", "Outlying-US(Guam-USVI-etc)", "Mexico", "Puerto-Rico", "Dominican-Republic", "Haiti", "Guatemala", "Cuba","Honduras", "Jamaica", "Nicaragua", "El-Salvador"), SouthA = c("Trinadad&Tobago", "Ecuador", "Columbia", "Peru"), Asia = c("Cambodia", "China", "Vietnam", "Laos", "Taiwan", "Thailand", "Hong", "Japan", "Philippines", "India", "South", "Iran"), Europe=c("England", "Germany", "Italy","Portugal", "Poland","Holand-Netherlands", "Yugoslavia", "Greece", "Ireland", "France", "Hungary", "Scotland"))
xx$occupation <- fct_collapse(xx$occupation, other = "Other-service", defense=c("Armed-Forces", "Protective-serv"), desk=c("Adm-clerical", "Exec-managerial", "Sales", "Prof-specialty", "Tech-support"), labor=c("Farming-fishing", "Handlers-cleaners", "Machine-op-inspct", "Priv-house-serv", "Craft-repair", "Transport-moving") )
xx$relationship <- fct_collapse(xx$relationship, family=c("Husband", "Wife", "Own-child", "Other-relative"), not_family="Not-in-family", single="Unmarried")
...
The highest accuracy seen in the decision tree model is 81%. This is pretty good but the Kappa value associated with that accuracy is 0.38 which is considered a moderate agreement for the data.
...
Based on the c5.0 variable importance, the capital gain was the most important variable for the dataset, and capital loss as well as marital status were considered less important. 
...
Looking at the metrics, the prevalence that we calculated in an earlier section for the population that has a salary above 50k was 75%. The model also calculated a prevalence of 75%. The Kappa value was measured to be 0.51 which is still considered a moderate interrater agreement. The positive predicted value was calculated to be 88.57% which is a good rate for the positive class as we want to have a higher true positive rate to be able to accurately predict those with a higher salary than 50k. The balanced accuracy was calculated to be 76.17% which is alright for an imbalanced data set. Important evaluation metrics for this data set include the F1 score and Kappa score. 
...
When the adjust threshold function is used, The values stayed the same as the ones that were calculated in the confusion matrix above. However, the F1 score was calculated to be 0.87 which is a good indicator that the model fits the data well. The positive predicted value is also a high number of 88.57%, further showing that more true positives are being found.

The plotted ROC curve unfortunately does not show a good true positive rate. Instead, it shows that the false positive rate has a greater effect on the model, which is not what we want. The AUC value further validates this as it is very low at 0.179. Due to these metrics, I would say that the performance of this model is not as good as I had previously thought. The threshold level was set to a 0.6 and may change the effectiveness of a model if the threshold level is changed.
...
When the threshold is adjusted to 0.4, The Kappa value decreases to 0.1976 but the F1 score is similar at 0.87 to when the threshold value was 0.6. The balanced accuracy was 0.57 which is not ideal and I would not recommend using a threshold of 0.4

When the threshold is adjusted to 0.8, the balanced accuracy improves to be 0.76. But when analyzing the confusion matrix, the number of false positives is higher than a threshold that is lower. The F1 score decreased to 0.78 and the positive predicted value is 0.92 which objectively is good, but taking into account all of the measures included, does not mean that this model is the most optimal.

To find a good balance between a threshold of 0.4 and 0.6, I chose to use 0.5 as the threshold. The Kappa value increased to 0.51 which is a moderate level and the F1 score was similar to the threshold of 0.6 where it is 0.87. The balanced accuracy also increased to 0.76 which is similar to the threshold of 0.8. It seems like the pattern is that the model finds more false positives when the threshold increases and is able to capture more true positives when the threshold is lower. 
...
The model for the second set of modified features had values where trials = 1, model = tree, and winnow = FALSE. This model had an accuracy of 0.86 and the Kappa value was higher than the other model and had a value of 0.6, showing that the data was in moderate agreement.  It seems like this model also improved by some amount and having a good Kappa value is important. 
...
For the first evaluated model, the positive predicted value had a similar rate to the previous models such as 88%. The balanced accuracy was somewhat better with 78% and this was a good sign for the model as the data was unbalanced. 

For the second evaluated model, the models had the same numbers as the evaluation metrics, which makes it seem like there was not a lot of modification in the train control package. It seems like both models predicted similar outcomes. 
