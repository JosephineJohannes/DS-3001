...
library(class)
library(plotly)
>>>>>>> c024dccc13659741a8579fe16790297b39dedde9
```
...
# Check the structure and view the data.
table(bank_data$`signed up`)
#check for missing data (just keeps the complete cases)
bank_data <- bank_data[complete.cases(bank_data),]
...
# create data partition for tuning and testing 
...
# you should use set.seed() if you want to use the data partition again 
...
set.seed(1982)
#passing in the train and test set but input tune for the testing dataset 
...
Building a kNN model with  groups recommended k value 
```{r}
# where k = 5
set.seed(1982)
#passing in the train and test set but input tune for the testing dataset 
...
# Let's take a look at the confusion matrix by combining the 
# predictions from bank_3NN to the original data set.
k5NN_res = table(bank_5NN,
                tune$`signed up`)
k5NN_res
sum(k5NN_res)  #<- the total is all the test examples

k5NN_res[row(k5NN_res) == col(k5NN_res)]

k5NN_acc = sum(k5NN_res[row(k5NN_res) == col(k5NN_res)]) / sum(k5NN_res)
k5NN_acc
   # the accuracy is 88% which is higher than 87%
...
set.seed(1982)
#passing in the train and test set but input tune for the testing dataset 
bank_4NN <-  knn(train = train[, c("age", "balance", "duration")],#<- training set cases
               test = tune[, c("age", "balance", "duration")],    #<- test set cases
               cl = train$`signed up`,#<- category for true classification
...
# Let's take a look at the confusion matrix by combining the 
# predictions from bank_3NN to the original data set.
k4NN_res = table(bank_4NN,
                tune$`signed up`)
k4NN_res
sum(k4NN_res)  #<- the total is all the test examples

k4NN_res[row(k4NN_res) == col(k4NN_res)]

k4NN_acc = sum(k4NN_res[row(k4NN_res) == col(k4NN_res)]) / sum(k4NN_res)
k4NN_acc
   # the accuracy is 86.965% which is lower than 87%
confusionMatrix(as.factor(bank_4NN), as.factor(tune$`signed up`), positive = "1", dnn=c("Prediction", "Actual"), mode = "sens_spec")
=======
# In Class Exercise 
```{r}
...
#passing in the train and test set but input tune for the testing dataset 
k_set <- (2:7)
k_accuracy <- function(x){
bank_4NN <-  knn(train = train[, c("age", "balance", "duration")],#<- training set cases
               test = tune[, c("age", "balance", "duration")],    #<- test set cases
               cl = train$`signed up`,#<- category for true classification
               k = x,#<- number of neighbors considered
               use.all = TRUE, # use all the data
               prob = TRUE) #<- control ties between class assignments If true, all distances equal to the kth largest are included
#when prob = TRUE, we want all of the probability
# View the output.
...
Could change the data set for a more robust 
